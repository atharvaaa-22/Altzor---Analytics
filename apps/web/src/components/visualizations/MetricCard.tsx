...
export function MetricCard({ title, value, trend, valueFormat }: MetricCardProps) {
  const isPositive = trend !== undefined && trend > 0;
  const isNegative = trend !== undefined && trend < 0;

  return (
    <div className="flex flex-col justify-center h-full p-2">
      <h3 className="text-sm font-medium text-slate-400 mb-2 truncate" title={title}>{title}</h3>
      <div className="flex items-end gap-3 flex-wrap">
        ...
        {trend !== undefined && (
          <div className={`flex items-center gap-1 text-sm font-medium mb-1 shrink-0 ${
