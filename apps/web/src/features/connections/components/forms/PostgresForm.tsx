...
export function PostgresForm({ type, onClose }: { type: ConnectionType; onClose: () => void }) {
  const { createConnection, testConnection } = useConnections();
  const [name, setName] = useState('');
  const [host, setHost] = useState('');
  const [port, setPort] = useState(type === 'mysql' ? '3306' : type === 'sqlserver' ? '1433' : '5432');
  ...
  const handleTest = async () => {
    ...
    try {
      await testConnection.mutateAsync({
        type,
        config: { host, port: parseInt(port), database, username, password, ssl },
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input required type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-2 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="e.g. Production DB" />
      </div>

      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-2">
          <label className="block text-sm font-medium text-slate-400 mb-1">Host</label>
          <input required type="text" value={host} onChange={(e) => setHost(e.target.value)} className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-2 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="e.g. db.example.com" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-400 mb-1">Port</label>
          <input required type="number" value={port} onChange={(e) => setPort(e.target.value)} className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-4 py-2 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mt-2">
        <input type="checkbox" id="ssl" checked={ssl} onChange={(e) => setSsl(e.target.checked)} className="rounded border-slate-700 bg-slate-900 text-blue-500 focus:ring-blue-500" />
        <label htmlFor="ssl" className="text-sm text-slate-400 cursor-pointer">Require SSL/TLS</label>
      </div>
