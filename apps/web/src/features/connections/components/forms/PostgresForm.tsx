...
export function PostgresForm({
  ...
  const [port, setPort] = useState(
    type === 'mysql' ? '3306' : type === 'sqlserver' ? '1433' : '5432',
  );
  ...
  const handleTest = async (): Promise<void> => {
    ...
    try {
      ...
      setTestStatus('error');
      setTestMessage((err as { message?: string }).message || 'Connection failed');
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mt-2">
        ...
        />
        <label htmlFor="ssl" className="text-sm text-slate-400 cursor-pointer">
          Require SSL/TLS
        </label>
      </div>
