  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wider">Token Usage Preview</h3>
          <p className="text-xs text-slate-500 mt-1">Estimated token consumption for current billing cycle.</p>
        </div>
      ...
      <div className="h-[200px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
            ...
            <YAxis stroke="#64748b" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(val) => `${(val as number)/1000}k`} />
            <Tooltip 
              cursor={{ fill: '#1e293b' }}
