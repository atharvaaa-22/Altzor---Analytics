...
export function PromptEditor({ prompt, setPrompt }: { prompt: string, setPrompt: (v: string) => void }) {
  ...
  const handleTest = () => {
    setIsTesting(true);
    setTimeout(() => {
      setTestResult(`SELECT\n  DATE_TRUNC('month', order_date) AS order_month,\n  SUM(total_amount) AS revenue\nFROM\n  orders\nWHERE\n  order_date >= CURRENT_DATE - INTERVAL '1 year'\nGROUP BY\n  1\nORDER BY\n  1 DESC;`);
      setIsTesting(false);
    }, 1500);
  };

  const highlightTemplateVariables = (text: string) => {
    return text.replace(/(\{\{[^}]+\}\})/g, '<span class="text-amber-400 font-bold bg-amber-400/10 px-1 rounded border border-amber-400/20">$1</span>');
  };
          ...
          <div className="h-full flex flex-col bg-slate-900 border-t border-slate-800">
            ...
            <div className="flex-1 flex flex-col sm:flex-row min-h-0">
              <div className="flex-1 sm:border-r border-b sm:border-b-0 border-slate-800 p-4 flex flex-col min-h-[150px]">
                <label className="text-xs text-slate-500 mb-2 uppercase tracking-wider font-semibold">User Query Input</label>
                <textarea 
                  value={testQuery}
              ...
              <div className="flex-1 p-4 flex flex-col bg-slate-950 min-h-[150px]">
                <label className="text-xs text-slate-500 mb-2 uppercase tracking-wider font-semibold flex justify-between">
                  ...
                  {isTesting && <span className="text-blue-400 animate-pulse font-normal normal-case">Generating...</span>}
                </label>
                <div className="flex-1 bg-slate-900 border border-slate-800 rounded-lg p-3 text-sm font-mono text-emerald-400 overflow-auto whitespace-pre scrollbar-thin">
                  {testResult || <span className="text-slate-600 italic">Click test to generate SQL...</span>}
                </div>
              </div>
            </div>
          </div>
