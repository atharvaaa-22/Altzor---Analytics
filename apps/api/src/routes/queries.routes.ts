...
const saveSchema = z.object({
  ...
  templateParams: z.record(z.string(), z.object({
    type: z.string(),
    label: z.string(),
...
router.post('/', async (req: Request, res: Response) => {
  ...
  res.status(201).json({
    ...
    templateParams: query.templateParams ? JSON.parse(query.templateParams) : null,
    chartConfig: query.chartConfig ? JSON.parse(query.chartConfig) : null,
  });
});
